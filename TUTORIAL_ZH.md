...
* 多核支持。Leaf 通过模块机制和 [leaf/go](https://github.com/qusuo/leaf/tree/master/go) 尽可能的利用多核资源，同时又尽量避免各种副作用
* 模块机制。

Leaf 的模块机制
---------------

一个 Leaf 开发的游戏服务器由多个模块组成（例如 [LeafServer](https://github.com/qusuo/leafserver)），模块有以下特点：

* 每个模块运行在一个单独的 goroutine 中
* 模块间通过一套轻量的 RPC 机制通讯（[leaf/chanrpc](https://github.com/qusuo/leaf/tree/master/chanrpc)）
...
[LeafServer](https://github.com/qusuo/leafserver) 是一个基于 Leaf 开发的游戏服务器，我们以 LeafServer 作为起点。
...
更加详细的用法可以参考 [leaf/chanrpc](https://github.com/qusuo/leaf/blob/master/chanrpc)。需要注意的是，无论封装多么精巧，跨 goroutine 的调用总不能像直接的函数调用那样简单直接，因此除非必要我们不要构建太多的模块，模块间不要太频繁的交互。模块在 Leaf 中被设计出来最主要是用于划分功能而非利用多核，Leaf 认为在模块内按需使用 goroutine 才是多核利用率问题的解决之道。
...
更加详细的用法可以参考 [leaf/go](https://github.com/qusuo/leaf/blob/master/go)。
...
更加详细的用法可以参考 [leaf/timer](https://github.com/qusuo/leaf/blob/master/timer)。
...
更加详细的用法可以参考 [leaf/log](https://github.com/qusuo/leaf/blob/master/log)。

### Leaf recordfile

Leaf 的 recordfile 是基于 CSV 格式（范例见[这里](https://github.com/qusuo/leaf/blob/master/recordfile/test.txt)）。recordfile 用于管理游戏配置数据。在 LeafServer 中使用 recordfile 非常简单：
...
更加详细的用法可以参考 [leaf/recordfile](https://github.com/qusuo/leaf/blob/master/recordfile)。

了解更多
---------------

阅读 Wiki 获取更多的帮助：[https://github.com/qusuo/leaf/wiki](https://github.com/qusuo/leaf/wiki)
